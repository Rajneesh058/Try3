...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        stream_link = btn = [[InlineKeyboardButton(text= f"[{(get_media_file_size(m))}] {file.file_name}", url = await get_shortlink(f"{Var.URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"))]]
        online_link = btn = [[InlineKeyboardButton(text=f"[{(get_media_file_size(m)e)}] {file.file_name}", url = await get_shortlink(f"{Var.URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"))]]
